...
type GoogleAnalysis3TipsEmailInput = {
  name: string;
  email: string;
  phone: string;
  website: string;
...
export function renderGoogleAnalysis3TipsCustomerHtml(name: string) {
  return `
    <div style="font-family: Inter, Arial, sans-serif; color: #1f1f1f; line-height: 1.6;">
      ...
      <p style="margin: 0 0 12px;">
        Благодарим ти за заявката за безплатен анализ с 3 съвета.
      </p>
      <p style="margin: 0 0 12px;">
        Екипът на DigiStart преглежда Google Business профила ти и скоро ще получиш
        3 конкретни неща, които можеш да промениш за по-високо класиране.
      </p>
...
export function renderGoogleAnalysis3TipsAdminHtml(input: GoogleAnalysis3TipsEmailInput) {
  const urgency = getGoogleFreeAnalysisUrgencyLabel(input.urgency);
  return `
    <div style="font-family: Inter, Arial, sans-serif; color: #1f1f1f; line-height: 1.6;">
      <h2 style="margin: 0 0 16px;">Нова заявка: Анализ 3 съвета</h2>
...
function renderAdminText(input: GoogleAnalysis3TipsEmailInput) {
  return [
    "Нова заявка: Анализ 3 съвета",
    `Име: ${input.name}`,
    `Имейл: ${input.email}`,
    `Телефон: ${input.phone}`,
...
export async function sendGoogleAnalysis3TipsLeadEmails(
  input: GoogleAnalysis3TipsEmailInput,
) {
  ...
  const customerSubject = withTestSubject(
    "Получихме заявката ти за Анализ 3 съвета",
    delivery.testMode,
  );
  const adminSubject = withTestSubject(
    `Нова заявка: Анализ 3 съвета - ${input.email}`,
    delivery.testMode,
  );
  ...
  const sends = await Promise.allSettled([
    transporter.sendMail({
      ...
      text: withTestTextBody(
        "Благодарим ти! Получихме заявката ти за Анализ 3 съвета.",
        delivery.testMode,
        { originalTo: input.email },
      ),
      html: withTestHtmlBody(renderGoogleAnalysis3TipsCustomerHtml(input.name), delivery.testMode, {
        originalTo: input.email,
      }),
    }),
    transporter.sendMail({
      ...
      html: withTestHtmlBody(renderGoogleAnalysis3TipsAdminHtml(input), delivery.testMode, {
        originalTo: notifyEmail,
      }),
    }),
  ]);
  ...
  if (sends.some((result) => result.status === "rejected")) {
    throw new Error("One or more analysis 3 tips emails failed to send.");
  }
}
