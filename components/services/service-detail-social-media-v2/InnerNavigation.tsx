...
function getActiveSectionId(): (typeof SOCIAL_MEDIA_SECTION_NAV)[number]["id"] {
  let currentId: (typeof SOCIAL_MEDIA_SECTION_NAV)[number]["id"] = SOCIAL_MEDIA_SECTION_NAV[0].id;

  for (const { id } of SOCIAL_MEDIA_SECTION_NAV) {
...
const InnerNavigation = () => {
  ...
  const [activeId, setActiveId] = useState<(typeof SOCIAL_MEDIA_SECTION_NAV)[number]["id"]>(
    SOCIAL_MEDIA_SECTION_NAV[0].id,
  );
  ...
  useEffect(() => {
    ...
    const sections = SOCIAL_MEDIA_SECTION_NAV.map(({ id }) => document.getElementById(id)).filter(
