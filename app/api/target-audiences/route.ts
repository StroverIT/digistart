...
import { z } from "zod";
import { createTargetAudienceLead } from "@/lib/server/target-audience-leads";
...
const payloadSchema = z.object({
  ...
  website: z.string().trim().min(2, "Въведете уебсайт или магазин."),
...
export async function POST(req: Request) {
  try {
    ...
    const result = await createTargetAudienceLead(parsed.data);
    if (result.status !== "ok") {
